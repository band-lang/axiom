...
typedef struct {
    ...
    uint16_t base_mid; // Mid 16 bits of handler address
    uint32_t base_high; // High 32 bits of handler address
    uint32_t reserved2;
} __attribute__((packed)) idt_entry_t;

// Functions
void idt_init(void);
void idt_set_gate(int n, uintptr_t handler, uint16_t flags);
